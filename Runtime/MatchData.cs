...
namespace Edgegap.Matchmaking
{
    ...
    public class MatchData<T, A, E, I>
            ...
            try
            {
                ...
                foreach (DictionaryEntry envEntry in envs)
                {
                    ...
                    if (key.StartsWith("MM_TICKET_") && !key.Contains("_IDS"))
                    {
                        ...
                        TicketsData[id] = JsonConvert.DeserializeObject<T>(
                    ...
                    {
                        Equality = JsonConvert.DeserializeObject<E>(envEntry.Value.ToString());
                    }
                    else if (key == "MM_INTERSECTION")
                    {
                        Intersection = JsonConvert.DeserializeObject<I>(envEntry.Value.ToString());
                    }
